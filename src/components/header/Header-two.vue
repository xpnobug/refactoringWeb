<template>
  ...
  <div class="body">
    ...
    <div class="details" id="details-even">
      ...
      <div class="desc">
        藏在瑞士阿尔卑斯山，圣Antönien提供了一个田园诗般的撤退为那些寻求宁静和
        冒险。这里是冬季野外滑雪的隐蔽之地，也有郁郁葱葱的小径可供远足和登山
        在温暖的月份骑自行车。
      </div>
      <div class="cta">
...
linkElement.href = '/src/assets/css/header-two.css'; // 根据实际路径调整
...
onMounted(() => {
  ...
  gsapScript.onload = () => {
    ...
    // if (!indicatorElement) {
    //   console.error('GSAP target .indicator not found.');
    //   return;
    // }

    // GSAP 脚本加载后，加载 header-two.js
    // const headerTwoScript = document.createElement('script');
    // headerTwoScript.src = '/src/assets/js/header-two.js'; // 根据实际路径调整
    // document.head.appendChild(headerTwoScript);
    ...
    async function loop() {
      ...
      await animate(".tiao", 2, { x: 0 });
      await animate(".tiao", 0.8, { x: window.innerWidth, delay: 0.3 });
      set(".tiao", { x: -window.innerWidth });
      await step();
      loop();
    }
